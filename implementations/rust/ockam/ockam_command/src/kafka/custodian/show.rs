...
impl Command for ShowCommand {
    const NAME: &'static str = "kafka-custodian show";
...
impl<'a> ShowCommandTui for ShowTui<'a> {
    ...
    async fn get_arg_item_name_or_default(&self) -> miette::Result<String> {
        Ok(self
            .cmd_arg_item_name()
            .unwrap_or(DefaultAddress::KAFKA_CUSTODIAN.to_string()))
    }
    ...
    async fn show_single(&self, item_name: &str) -> miette::Result<()> {
        ...
        let status = instances
            ...
            .ok_or_else(|| miette!("Kafka Custodian not found"))?;
        self.terminal()
            .stdout()
            .plain(status.item()?)
            .json_obj(&status)?
